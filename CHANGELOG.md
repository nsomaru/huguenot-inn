...
- Added separator rows to all bundle/index outputs, with an **Add Separator** button and mixed PDF/separator edit, move, and remove support.
- Added internal PDF page labels for front-index bundles: roman labels for index pages and Arabic numbering restarted for bundled documents.

### Changed

- Routed macOS Command+Q through the same close path as the window close/File Exit action.
- Preserved generated icon files when regenerated bytes are unchanged.

### Tests

- Added regression coverage for separator numbering/rendering, PDF outlines/page labels, icon checksum preservation, macOS quit wiring, and static local-network packaging audit checks.
